...
type DanielUpwardPressApiResponse = {
  motion_id: string
  state: string
  accuracy: number
  ...
  features: {
    ...
    wrist_height_balance: number | null
...
function DanielUpwardPressDebugPage() {
  ...
  const [result, setResult] = useState<DanielUpwardPressApiResponse | null>(null)
  ...
  const startTracking = async () => {
    ...
    try {
      ...
      resetEvaluatorState()
      setRunning(true)
      setStatus('Daniel upward press test running')
      ...
      const loop = async () => {
        ...
        if (pose && !requestInFlightRef.current) {
          ...
          try {
            const response = await fetch(`${aiBaseUrl}/gymnastics/daniel-upward-press/evaluate`, {
              method: 'POST',
            ...
            const payload = (await response.json()) as DanielUpwardPressApiResponse
  ...
  return (
    <div style={pageStyle}>
      <div style={layoutStyle}>
        <section>
          <h1 style={{ margin: 0, fontSize: '32px' }}>Daniel Upward Press AI Debug</h1>
          <p style={{ marginTop: '8px', color: '#4d5b71' }}>
            Capture pose landmarks from the webcam, send them to the `daniel-upward-press/evaluate`
            API, and inspect hold-based upward press evaluation in real time.
          </p>
          ...
          <div style={guideStyle}>
            Start를 누른 뒤 0.5초 정도 차렷 자세로 가만히 서서 기준 자세를 잡습니다. 그다음 손
            깍지를 끼고 손바닥을 위로 향하게 한 채 머리 위로 쭉 올려서 자세를 유지하면 `holding`으로
            들어가고 시간이 누적됩니다.
          </div>
        ...
        <section style={panelStyle}>
          ...
          <p style={{ marginTop: 0, marginBottom: '16px', color: '#677489', fontSize: '14px' }}>
            ...
            <strong>feedback</strong>, <strong>wristHeight</strong>,{' '}
            <strong>wristHeightBalance</strong>
          </p>
          ...
          <InfoRow label="wristHeight" value={formatNumber(result?.features.wrist_height)} />
          <InfoRow
            label="wristHeightBalance"
            value={formatNumber(result?.features.wrist_height_balance)}
          />
...
export default DanielUpwardPressDebugPage
