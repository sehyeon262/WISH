...
type DanielLeftSideBendApiResponse = {
  motion_id: string
  state: string
  accuracy: number
...
function DanielLeftSideBendDebugPage() {
  ...
  const [result, setResult] = useState<DanielLeftSideBendApiResponse | null>(null)
  ...
  const startTracking = async () => {
    ...
    try {
      ...
      resetEvaluatorState()
      setRunning(true)
      setStatus('Daniel left side bend test running')
      ...
      const loop = async () => {
        ...
        if (pose && !requestInFlightRef.current) {
          ...
          try {
            const response = await fetch(`${aiBaseUrl}/gymnastics/daniel-left-side-bend/evaluate`, {
              method: 'POST',
            ...
            const payload = (await response.json()) as DanielLeftSideBendApiResponse
  ...
  return (
    <div style={pageStyle}>
      <div style={layoutStyle}>
        <section>
          <h1 style={{ margin: 0, fontSize: '32px' }}>Daniel Left Side Bend AI Debug</h1>
          <p style={{ marginTop: '8px', color: '#4d5b71' }}>
            Capture pose landmarks from the webcam, send them to the
            `daniel-left-side-bend/evaluate` API, and inspect hold-based left side bend evaluation
            in real time.
          </p>
          ...
          <div style={guideStyle}>
            Start를 누른 뒤 0.5초 정도 차렷 자세로 가만히 서세요. 그 다음 양손을 머리 위로 올린
            상태에서 몸통을 왼쪽으로 충분히 기울이면 `holding`으로 들어가고, 10초를 유지하면
            `complete`가 됩니다.
          </div>
...
export default DanielLeftSideBendDebugPage
