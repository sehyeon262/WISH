...
type SideStepApiResponse = {
  ...
  features: {
    left_step_extent: number
    right_step_extent: number
    ankle_span: number
    torso_tilt: number
...
function SideStepDebugPage() {
  ...
  const baselineLeftStepExtentRef = useRef<number | null>(null)
  const baselineRightStepExtentRef = useRef<number | null>(null)
  const baselineAnkleSpanRef = useRef<number | null>(null)
  ...
  const [result, setResult] = useState<SideStepApiResponse | null>(null)
  ...
  const resetEvaluatorState = () => {
    ...
    baselineLeftStepExtentRef.current = null
    baselineRightStepExtentRef.current = null
    baselineAnkleSpanRef.current = null
  ...
  const startTracking = async () => {
    ...
    try {
      ...
      resetEvaluatorState()
      setRunning(true)
      setStatus('Side step test running')
      ...
      const loop = async () => {
        ...
        if (pose && !requestInFlightRef.current) {
          ...
          try {
            const response = await fetch(`${aiBaseUrl}/gymnastics/side-step/evaluate`, {
              method: 'POST',
              ...
              body: JSON.stringify({
                ...
                baseline_left_step_extent: baselineLeftStepExtentRef.current,
                baseline_right_step_extent: baselineRightStepExtentRef.current,
                baseline_ankle_span: baselineAnkleSpanRef.current,
            ...
            const payload = (await response.json()) as SideStepApiResponse
            ...
            baselineLeftStepExtentRef.current = payload.baseline_left_step_extent
            baselineRightStepExtentRef.current = payload.baseline_right_step_extent
            baselineAnkleSpanRef.current = payload.baseline_ankle_span
  ...
  return (
    ...
    >
      ...
      >
        <section>
          <h1 style={{ margin: 0, fontSize: '32px' }}>Side Step AI Debug</h1>
          <p style={{ marginTop: '8px', color: '#4d5b71' }}>
            Capture pose landmarks from the webcam, send them to the `side-step/evaluate` API, and
            inspect alternating lateral step counting in real time.
          </p>
        ...
        >
          ...
          <p style={{ marginTop: 0, marginBottom: '16px', color: '#677489', fontSize: '14px' }}>
            ...
            <strong>feedback</strong>, <strong>leftStepExtent</strong>,{' '}
            <strong>rightStepExtent</strong>, <strong>ankleSpan</strong>
          </p>
          ...
          <InfoRow label="leftStepExtent" value={formatNumber(result?.features.left_step_extent)} />
          <InfoRow
            label="rightStepExtent"
            value={formatNumber(result?.features.right_step_extent)}
          />
          <InfoRow label="ankleSpan" value={formatNumber(result?.features.ankle_span)} />
          ...
          {showAdvanced ? (
            <>
              ...
              <InfoRow
                label="baselineLeftStepExtent"
                value={formatNumber(result?.baseline_left_step_extent)}
              />
              <InfoRow
                label="baselineRightStepExtent"
                value={formatNumber(result?.baseline_right_step_extent)}
              />
              <InfoRow
                label="baselineAnkleSpan"
                value={formatNumber(result?.baseline_ankle_span)}
              />
...
export default SideStepDebugPage
