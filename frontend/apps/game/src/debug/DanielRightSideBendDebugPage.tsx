...
type DanielRightSideBendApiResponse = {
  motion_id: string
  state: string
  accuracy: number
...
function DanielRightSideBendDebugPage() {
  ...
  const [result, setResult] = useState<DanielRightSideBendApiResponse | null>(null)
  ...
  const startTracking = async () => {
    ...
    try {
      ...
      resetEvaluatorState()
      setRunning(true)
      setStatus('Daniel right side bend test running')
        ...
        if (pose && !requestInFlightRef.current) {
          ...
          try {
            const response = await fetch(
              `${aiBaseUrl}/gymnastics/daniel-right-side-bend/evaluate`,
              {
                method: 'POST',
            ...
            const payload = (await response.json()) as DanielRightSideBendApiResponse
  ...
  return (
    <div style={pageStyle}>
      <div style={layoutStyle}>
        <section>
          <h1 style={{ margin: 0, fontSize: '32px' }}>Daniel Right Side Bend AI Debug</h1>
          <p style={{ marginTop: '8px', color: '#4d5b71' }}>
            Capture pose landmarks from the webcam, send them to the
            `daniel-right-side-bend/evaluate` API, and inspect hold-based right side bend evaluation
            in real time.
          </p>
          ...
          <div style={guideStyle}>
            Start를 누른 뒤 0.5초 정도 차렷 자세로 기준 자세를 잡고, 양손을 머리 위로 올린 상태에서
            허리를 오른쪽으로 꾸준히 기울여 보세요. 손 높이와 오른쪽 기울기가 모두 유지되면
            `holding`이 되고 10초를 채우면 `complete`가 됩니다.
          </div>
        ...
        <section style={panelStyle}>
          ...
          <p style={{ marginTop: 0, marginBottom: '16px', color: '#677489', fontSize: '14px' }}>
            ...
            <strong>feedback</strong>, <strong>torsoTilt</strong>, <strong>wristHeight</strong>
          </p>
...
export default DanielRightSideBendDebugPage
