...
type DanielForwardBendApiResponse = {
  motion_id: string
  state: string
  accuracy: number
  ...
  features: {
    forward_bend_angle: number
    wrist_drop: number | null
...
function DanielForwardBendDebugPage() {
  ...
  const [result, setResult] = useState<DanielForwardBendApiResponse | null>(null)
  ...
  const startTracking = async () => {
    ...
    try {
      ...
      resetEvaluatorState()
      setRunning(true)
      setStatus('Daniel forward bend test running')
      ...
      const loop = async () => {
        ...
        if (pose && !requestInFlightRef.current) {
          ...
          try {
            const response = await fetch(`${aiBaseUrl}/gymnastics/daniel-forward-bend/evaluate`, {
              method: 'POST',
            ...
            const payload = (await response.json()) as DanielForwardBendApiResponse
  ...
  return (
    <div style={pageStyle}>
      <div style={layoutStyle}>
        <section>
          <h1 style={{ margin: 0, fontSize: '32px' }}>Daniel Forward Bend AI Debug</h1>
          <p style={{ marginTop: '8px', color: '#4d5b71' }}>
            Capture pose landmarks from the webcam, send them to the `daniel-forward-bend/evaluate`
            API, and inspect hold-based forward bend evaluation in real time.
          </p>
          ...
          <div style={guideStyle}>
            Start를 누른 뒤 0.5초 정도 가만히 서서 기준 자세를 잡고, 상체를 앞으로 충분히 숙인 뒤
            손을 아래로 내려보세요. 자세가 맞으면 `holding`, 10초 유지하면 `complete`가 됩니다.
          </div>
        ...
        <section style={panelStyle}>
          ...
          <p style={{ marginTop: 0, marginBottom: '16px', color: '#677489', fontSize: '14px' }}>
            ...
            <strong>feedback</strong>, <strong>forwardBendAngle</strong>, <strong>wristDrop</strong>
          </p>
          ...
          <InfoRow
            label="forwardBendAngle"
            value={formatNumber(result?.features.forward_bend_angle)}
          />
          <InfoRow label="wristDrop" value={formatNullableNumber(result?.features.wrist_drop)} />
          ...
          {showAdvanced ? (
            <>
              ...
              <InfoRow
                label="leftKneeAngle"
                value={formatNullableAngle(result?.features.left_knee_angle)}
              />
              <InfoRow
                label="rightKneeAngle"
                value={formatNullableAngle(result?.features.right_knee_angle)}
              />
...
function formatNullableAngle(value: number | null | undefined) {
  if (value == null) return '-'
...
export default DanielForwardBendDebugPage
