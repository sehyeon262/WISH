...
type DiagonalBodyPunchApiResponse = {
  motion_id: string
  state: string
  step_count: number
  accuracy: number
...
function DiagonalBodyPunchDebugPage() {
  ...
  const [result, setResult] = useState<DiagonalBodyPunchApiResponse | null>(null)
  ...
  const startTracking = async () => {
    ...
    try {
      ...
      resetEvaluatorState()
      setRunning(true)
      setStatus('Diagonal body punch test running')
      ...
      const loop = async () => {
        ...
        if (pose && !requestInFlightRef.current) {
          ...
          try {
            const response = await fetch(`${aiBaseUrl}/gymnastics/diagonal-body-punch/evaluate`, {
              method: 'POST',
            ...
            const payload = (await response.json()) as DiagonalBodyPunchApiResponse
  ...
  return (
    ...
    >
      ...
      >
        <section>
          <h1 style={{ margin: 0, fontSize: '32px' }}>Diagonal Body Punch AI Debug</h1>
          <p style={{ marginTop: '8px', color: '#4d5b71' }}>
            Capture pose landmarks from the webcam, send them to the `diagonal-body-punch/evaluate`
            API, and inspect alternating diagonal punch counting in real time.
          </p>
        ...
        >
          ...
          <p style={{ marginTop: 0, marginBottom: '16px', color: '#677489', fontSize: '14px' }}>
            ...
            <strong>rightWristForward</strong>, <strong>stanceSpan</strong> (all shown as
            baseline-relative increases)
          </p>
...
export default DiagonalBodyPunchDebugPage
