...
class DanielLeftSideBendFeatureSet:
    torso_tilt: float
...
def extract_daniel_left_side_bend_features(
    ...
    torso_tilt = _compute_left_side_tilt(frame)
    ...
    return DanielLeftSideBendFeatureSet(
...
def _compute_left_side_tilt(frame: NormalizedPoseFrame) -> float:
    left_shoulder = frame.landmarks.get(LEFT_SHOULDER)
    right_shoulder = frame.landmarks.get(RIGHT_SHOULDER)
    ...
    return max(hip_center[0] - shoulder_center[0], 0.0)
