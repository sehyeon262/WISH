...
def test_daniel_right_side_bend_stays_idle_on_first_neutral_frame() -> None:
    evaluator = DanielRightSideBendEvaluator()

    result = evaluator.evaluate(
        frame=build_right_side_bend_frame(timestamp_ms=0, pose="neutral"),
        previous_state="idle",
        step_count=0,
        target_steps=1,
    )
...
def test_daniel_right_side_bend_holds_and_completes_after_target_duration() -> None:
    evaluator = DanielRightSideBendEvaluator()
    ...
    holding_result = evaluator.evaluate(
        frame=build_right_side_bend_frame(timestamp_ms=100, pose="right_bend"),
    ...
    complete_result = evaluator.evaluate(
        frame=build_right_side_bend_frame(timestamp_ms=250, pose="right_bend"),
...
def test_daniel_right_side_bend_sets_lean_right_candidate_when_tilt_is_too_small() -> None:
    evaluator = DanielRightSideBendEvaluator()
    ...
    weak_result = evaluator.evaluate(
        frame=build_right_side_bend_frame(timestamp_ms=100, pose="weak_right_bend"),
        previous_state=baseline_result.state,
        step_count=0,
        target_steps=1,
        reference_hip_x=baseline_result.reference_hip_x,
    ...
    assert weak_result.candidate_feedback_code == "LEAN_RIGHT_MORE"


def test_daniel_right_side_bend_sets_keep_hands_overhead_candidate_when_hands_are_low() -> None:
    evaluator = DanielRightSideBendEvaluator()
    ...
    low_hand_result = evaluator.evaluate(
        frame=build_right_side_bend_frame(timestamp_ms=100, pose="low_hands_right_bend"),
        previous_state=baseline_result.state,
        step_count=0,
        target_steps=1,
        reference_hip_x=baseline_result.reference_hip_x,
...
def test_daniel_right_side_bend_keeps_holding_when_elbow_angle_is_temporarily_missing() -> None:
    evaluator = DanielRightSideBendEvaluator()
    ...
    recovered_result = evaluator.evaluate(
        frame=build_right_side_bend_frame(timestamp_ms=200, pose="right_bend_missing_elbows"),
        previous_state=holding_result.state,
        step_count=0,
        target_steps=1,
        reference_hip_x=holding_result.reference_hip_x,
...
def build_right_side_bend_frame(
    *,
    timestamp_ms: int,
    pose: str,
) -> NormalizedPoseFrame:
    ...
    elif pose == "right_bend":
        landmarks = {
            "LEFT_SHOULDER": landmark("LEFT_SHOULDER", 0.02, -1.16, 0.0),
            "RIGHT_SHOULDER": landmark("RIGHT_SHOULDER", 0.92, -1.06, 0.0),
            "LEFT_ELBOW": landmark("LEFT_ELBOW", -0.08, -1.62, 0.02),
            "RIGHT_ELBOW": landmark("RIGHT_ELBOW", 0.68, -1.58, 0.02),
            "LEFT_WRIST": landmark("LEFT_WRIST", -0.18, -1.96, 0.03),
            "RIGHT_WRIST": landmark("RIGHT_WRIST", 0.58, -1.92, 0.03),
        }
    elif pose == "weak_right_bend":
        landmarks = {
            "LEFT_SHOULDER": landmark("LEFT_SHOULDER", -0.24, -1.12, 0.0),
            "RIGHT_SHOULDER": landmark("RIGHT_SHOULDER", 0.66, -1.08, 0.0),
            "LEFT_ELBOW": landmark("LEFT_ELBOW", -0.34, -1.60, 0.02),
            "RIGHT_ELBOW": landmark("RIGHT_ELBOW", 0.42, -1.58, 0.02),
            "LEFT_WRIST": landmark("LEFT_WRIST", -0.44, -1.92, 0.03),
            "RIGHT_WRIST": landmark("RIGHT_WRIST", 0.32, -1.90, 0.03),
        }
    elif pose == "low_hands_right_bend":
        landmarks = {
            "LEFT_SHOULDER": landmark("LEFT_SHOULDER", 0.00, -1.16, 0.0),
            "RIGHT_SHOULDER": landmark("RIGHT_SHOULDER", 0.90, -1.06, 0.0),
            "LEFT_ELBOW": landmark("LEFT_ELBOW", -0.08, -1.40, 0.02),
            "RIGHT_ELBOW": landmark("RIGHT_ELBOW", 0.68, -1.36, 0.02),
            "LEFT_WRIST": landmark("LEFT_WRIST", -0.18, -1.52, 0.03),
            "RIGHT_WRIST": landmark("RIGHT_WRIST", 0.58, -1.48, 0.03),
        }
    elif pose == "right_bend_missing_elbows":
        landmarks = {
            "LEFT_SHOULDER": landmark("LEFT_SHOULDER", 0.02, -1.16, 0.0),
            "RIGHT_SHOULDER": landmark("RIGHT_SHOULDER", 0.92, -1.06, 0.0),
            "LEFT_WRIST": landmark("LEFT_WRIST", -0.18, -1.96, 0.03),
